...
class ConvBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv_3_d2 = nn.Conv2d(in_channels, out_channels, kernel_size=3, padding='same', dilation=2)
        self.conv_5_d1 = nn.Conv2d(out_channels, out_channels, kernel_size=5, padding='same', dilation=1)
        self.conv_3_d1 = nn.Conv2d(out_channels, out_channels, kernel_size=3, padding='same', dilation=1)
...
class Unet(nn.Module):
    def __init__(self):
        super().__init__()
        self.conv_block_1 = ConvBlock(1, 8)
        self.down_block_1 = DownBlock()
        self.conv_block_2 = ConvBlock(8, 16)
        self.down_block_2 = DownBlock()
        self.conv_block_3 = ConvBlock(16, 32)
        self.down_block_3 = DownBlock()
        
        self.bridge = ConvBlock(32, 64)

        self.up_block_3 = UpBlock(64, 32)
        self.up_block_2 = UpBlock(32, 16)
        self.up_block_1 = UpBlock(16, 8)

        self.last_conv = ConvBlock(8, 1)
    ...
    def forward(self, x):
        ...
        x_conv_2 = self.conv_block_2(x_down_1)
        x_down_2 = self.down_block_2(x_conv_2)
        
        x_conv_3 = self.conv_block_3(x_down_2)
        x_down_3 = self.down_block_3(x_conv_3)
        
        x_bridge = self.bridge(x_down_3)
...
def unwrap_phase(filePath: str):
    model = Unet()
    w_dict = torch.load("./app/services/model.pt", weights_only=True, map_location=torch.device('cpu'))
